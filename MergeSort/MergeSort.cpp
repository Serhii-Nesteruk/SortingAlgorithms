...
void mergeSort(T(&arr1)[sizeArr], T(&arr2)[sizeArr], int i_p, int i_k) {
		int i_s = 0, i1 = 0, i2 = 0, i = 0;

		i_s = (i_p + i_k + 1) / 2;
		if (i_s - i_p > 1) mergeSort(arr1, arr2, i_p, i_s - 1);
		if (i_k - i_s > 0) mergeSort(arr1, arr2, i_s, i_k);
		i1 = i_p; i2 = i_s;
		for (i = i_p; i <= i_k; i++)
...
int main() {
	...
	mergeSort(arr1, arr2, 0, sizeArr - 1);
